#!/usr/bin/env python3
""" An example using tkinter and websocket connections. """
...
class NetMemApp():
    def __init__(self, root, connector: netmem.Connector):
        self.window = root
        root.title("NetMem {}".format(repr(connector)))
        ...
        self.netmem.connect_on_new_thread(connector)  # New thread to deconflict with tk event loop
...
def main():
    tk1 = tk.Tk()
    conn1 = netmem.WsServerConnector(port=9998)
